...
export default function Allocation({ sekolahId }: { sekolahId: number }) {
  ...
  useEffect(() => {
    loadData().catch((err) => setError(err instanceof Error ? err.message : 'Gagal memuat data'));
  }, [sekolahId]);

  const availableBooks = inventory.filter((item) => item.total_lisensi > item.teralokasi);
      ...
      <GlassCard>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px', maxWidth: '480px' }}>
          <label style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <span style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-secondary)' }}>ID / NISN Siswa</span>
            <input
              type="text"
              value={siswaId}
          ...
          <label style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <span style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-secondary)' }}>Buku</span>
            <select
              value={isbn}
          ...
          <ButtonPromax type="submit" disabled={loading || availableBooks.length === 0}>
              ...
              {allocations.length === 0 ? (
                ...
                allocations.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td>{row.judul}</td>
                    <td style={{ color: 'var(--text-secondary)' }}>{row.isbn}</td>
                    <td>{new Date(row.tanggal_alokasi).toLocaleDateString('id-ID')}</td>
                  </tr>
                ))
              )}
            </tbody>
