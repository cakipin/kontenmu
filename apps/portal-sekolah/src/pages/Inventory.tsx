...
export default function Inventory({ sekolahId }: { sekolahId: number }) {
  ...
  useEffect(() => {
    api.getInventory(sekolahId)
      .then(setData)
      .catch((err) => setError(err instanceof Error ? err.message : 'Gagal memuat inventaris'))
  ...
  return (
    <GlassCard style={{ width: '100%', maxWidth: '1000px' }}>
      ...
      {loading && <p style={{ color: 'var(--text-secondary)' }}>Memuat data...</p>}
              ...
              {data.map((item) => {
                ...
                return (
                  <tr key={item.isbn}>
                    ...
                    <td>
                      <Chip type={sisa > 0 ? 'success' : 'warning'} label={sisa > 0 ? 'Tersedia' : 'Habis'} />
                    </td>
                  </tr>
                );
              })}
