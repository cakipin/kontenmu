...
export default function Dashboard({ sekolahId }: { sekolahId: number }) {
  ...
  const totalLisensi = inventory.reduce(
    (sum, item) => sum + item.total_lisensi,
    0,
  );
  const totalTeralokasi = inventory.reduce(
    (sum, item) => sum + item.teralokasi,
    0,
  );
    ...
    <div style={{ maxWidth: "1200px", width: "100%", margin: "0 auto" }}>
      <div className="dashboard-grid">
        <MetricCard
          ...
          color="#4f46e5"
          title="Total Lisensi"
          value={String(totalLisensi)}
          subtitle="Dari agen wilayah"
        />
        <MetricCard
          icon="👥"
          color="#10b981"
          title="Teralokasi"
          value={String(totalTeralokasi)}
          subtitle="Ke siswa aktif"
        />
        <MetricCard
          icon="📖"
          color="#0ea5e9"
                    ...
                    <tr key={item.isbn}>
                      ...
                      <td>
                        <Chip
                          ...
                          label={
                            sisaItem > 0 ? `${sisaItem} tersedia` : "Penuh"
                          }
                        />
                      </td>
                    </tr>
