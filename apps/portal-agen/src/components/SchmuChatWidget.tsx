...
export function SchmuChatWidget() {
  ...
  useEffect(() => {
    setMessages([
      {
        role: "assistant",
        content: data.aiWelcomeMessage || "Halo! Ada yang bisa saya bantu?",
      },
    ]);
...
${data.aiAutoContext || ""}`;
      ...
      if (data.aiProvider === "gemini") {
        ...
        const response = await fetch(
          `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${data.aiApiKey}`,
          {
            method: "POST",
        ...
        const response = await fetch(
          data.aiApiEndpoint || "https://schmu.id/api/chat/ask",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: "12px",
                  background:
                    isLoading || !inputValue.trim() ? "#94a3b8" : "#087f5b",
                  border: "none",
                  color: "#fff",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  cursor:
                    isLoading || !inputValue.trim() ? "default" : "pointer",
                  flexShrink: 0,
