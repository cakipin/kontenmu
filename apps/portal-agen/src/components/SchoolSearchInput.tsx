...
export function SchoolSearchInput({
  ...
  value: string;
  onChange: (val: string, id?: number, school?: any) => void;
  className?: string;
    ...
    if (searchQuery.length >= 3 && isOpen) {
      ...
      if (agentFilter && data && data.schools) {
        const timer = setTimeout(() => {
          ...
          const agentSchools = data.schools.filter(
            (school) => school.agen === agentFilter,
          );
          const results = agentSchools.filter(
            (s) =>
              s.nama.toLowerCase().includes(lowerQuery) ||
              (s.npsn && s.npsn.includes(lowerQuery)),
          );
        ...
        const timer = setTimeout(() => {
          ...
          const results = subscribedSchools.filter(
            (s) =>
              s.nama.toLowerCase().includes(lowerQuery) ||
              (s.npsn && s.npsn.includes(lowerQuery)),
          );
        ...
        const timer = setTimeout(() => {
          ...
          fetch(
            `${baseUrl}/api/sekolah?search=` + encodeURIComponent(searchQuery),
          )
            .then((res) => res.json())
            .then((resData) => {
