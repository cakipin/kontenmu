...
export function DeploymentManager() {
  ...
  const pushGit = async () => {
    if (!githubToken || !repoOwner || !repoName) {
      setMessage({
        text: "Harap lengkapi konfigurasi GitHub terlebih dahulu.",
        type: "error",
      });
      return;
    }
    ...
    setMessage({
      text: "Memulai Push Git (Repository Dispatch)...",
      type: "info",
    });
    ...
    try {
      ...
      if (response.ok || response.status === 204) {
        setMessage({
          text: "Push Git berhasil dipicu! Silakan cek tab Actions di GitHub Anda.",
          type: "success",
        });
      } else {
        const errData = await response.json().catch(() => ({}));
        setMessage({
          text: `Gagal Push Git: ${errData.message || response.statusText}`,
          type: "error",
        });
      }
    } catch (error: any) {
  ...
  const pushProduction = async () => {
    if (!githubToken || !repoOwner || !repoName) {
      setMessage({
        text: "Harap lengkapi Token PAT, Repo Owner, dan Repo Name untuk Push Production.",
        type: "error",
      });
      return;
    }
    ...
    setMessage({
      text: "Memulai Push Production (Repository Dispatch)...",
      type: "info",
    });
    ...
    try {
      ...
      if (response.ok || response.status === 204) {
        setMessage({
          text: "Push Production berhasil dipicu! Silakan cek tab Actions di GitHub Anda.",
          type: "success",
        });
      } else {
        const errData = await response.json().catch(() => ({}));
        setMessage({
          text: `Gagal Push Production: ${errData.message || response.statusText}`,
          type: "error",
        });
      }
    } catch (error: any) {
  ...
  const rollbackProduction = async () => {
    if (!githubToken || !repoOwner || !repoName) {
      setMessage({
        text: "Harap lengkapi Token PAT, Repo Owner, dan Repo Name untuk Rollback.",
        type: "error",
      });
      return;
    }
    ...
    if (
      !window.confirm(
        "PERINGATAN: Aksi ini akan membatalkan (undo) 1 update terakhir dan mengembalikan sistem ke versi stabil sebelumnya. Lanjutkan?",
      )
    ) {
      return;
    }
    ...
    setMessage({
      text: "Memulai Rollback Production (Membatalkan Update Terakhir)...",
      type: "info",
    });
    ...
    try {
      ...
      if (response.ok || response.status === 204) {
        setMessage({
          text: "Rollback berhasil dipicu! Silakan cek tab Actions di GitHub Anda.",
          type: "success",
        });
      } else {
        const errData = await response.json().catch(() => ({}));
        setMessage({
          text: `Gagal Rollback: ${errData.message || response.statusText}`,
          type: "error",
        });
      }
    } catch (error: any) {
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            <div>
              ...
              >
                Setup koneksi Git dan trigger deployment dari Dashboard
              </p>
            </div>
          </div>
          <div
          ...
          >
            <button
              ...
              style={{
                ...
                border: "none",
                background:
                  activeTab === "config" ? "var(--primary)" : "transparent",
                color:
                  activeTab === "config" ? "#fff" : "var(--text-secondary)",
                cursor: "pointer",
                fontSize: "0.875rem",
                fontWeight: 600,
                transition: "all 0.2s",
            ...
            <button
              ...
              style={{
                ...
                border: "none",
                background:
                  activeTab === "logs" ? "var(--primary)" : "transparent",
        ...
        {activeTab === "config" ? (
          <>
            ...
            >
              ...
              ></div>
              <a
                href="https://kontenmu-prod.pages.dev"
                target="_blank"
                rel="noreferrer"
                style={{
                  fontSize: "14px",
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        ...
                        >
                          <GitMerge size={12} /> {log.head_branch}
                        </span>
                      </div>
                    </div>
                  </div>
