...
const Allocation = lazy(() =>
  FeaturePages.then((m) => ({ default: m.Allocation })),
);
const TeacherAccess = lazy(() =>
  FeaturePages.then((m) => ({ default: m.TeacherAccess })),
);
const Catalog = lazy(() => FeaturePages.then((m) => ({ default: m.Catalog })));
const Inventory = lazy(() =>
  FeaturePages.then((m) => ({ default: m.Inventory })),
);
const LearningHistory = lazy(() =>
  FeaturePages.then((m) => ({ default: m.LearningHistory })),
);
const Library = lazy(() => FeaturePages.then((m) => ({ default: m.Library })));
const Payments = lazy(() =>
  FeaturePages.then((m) => ({ default: m.Payments })),
);
const PlayKonten = lazy(() =>
  FeaturePages.then((m) => ({ default: m.PlayKonten })),
);
const SchoolUsers = lazy(() =>
  FeaturePages.then((m) => ({ default: m.SchoolUsers })),
);
const Subscriptions = lazy(() =>
  FeaturePages.then((m) => ({ default: m.Subscriptions })),
);
const UploadContent = lazy(() =>
  FeaturePages.then((m) => ({ default: m.UploadContent })),
);
const MasterSekolah = lazy(() =>
  FeaturePages.then((m) => ({ default: m.MasterSekolah })),
);
const RoleAccessSettings = lazy(() =>
  FeaturePages.then((m) => ({ default: m.RoleAccessSettings })),
);
...
const MASTER_NAVIGATION: NavItemType[] = [
  ...
  },
  {
    id: "inventory",
    path: "/inventory",
    label: "Inventaris KontenMu",
    icon: "inventory",
  },
  {
    id: "allocation",
    path: "/allocation",
    label: "Alokasi Akses Siswa",
    icon: "allocation",
  },
  {
    id: "teacher-allocation",
    path: "/teacher-allocation",
    label: "Alokasi Akses Guru",
    icon: "schoolusers",
  },
  {
    id: "school-users",
...
const MOBILE_NAVIGATION: Record<
  string,
  Array<{ id: string; path: string; label: string; icon: NavIcon }>
> = {
  superadmin: [
  ...
  const paths: Record<string, ReactNode> = {
    ...
    users: (
      <>
        ...
        <path d="M21 19c0-1.8-1.2-3.2-3-3.8" />
    ...
    history: (
      <>
        <path d="M4 5v5h5" />
        <path d="M5 10a8 8 0 1 0 2.4-5.7" />
        <path d="M12 8v5l3 2" />
      </>
    ),
    payments: (
    ...
    schoolusers: (
      <>
        <path d="M16 18v-1.5A3.5 3.5 0 0 0 12.5 13h-5A3.5 3.5 0 0 0 4 16.5V18" />
...
function AppContent() {
  ...
  let navItems = MASTER_NAVIGATION.filter((item) => {
    ...
    if (item.subItems) {
      return item.subItems.some((sub) =>
        data.roleAccessPermissions?.[sub.id]?.includes(session.role),
      );
    }
  ...
  const mobileNavItems =
    MOBILE_NAVIGATION[session.role] ?? MOBILE_NAVIGATION.pending;

  return (
    <div className="admin-layout">
      <aside
        className={`admin-sidebar ${isSidebarOpen ? "is-open" : ""} ${isCollapsed ? "collapsed" : ""}`}
      >
          ...
          {navItems.map((item) => {
            if (item.subItems) {
              ...
              return (
                <div
                  key={item.id}
                  className={`admin-nav-group ${isOpen ? "is-open" : ""} ${isActive ? "active-group" : ""}`}
                >
                  <button
                    className={`admin-nav-item has-submenu ${isActive ? "active" : ""}`}
                  ...
                  >
                    ...
                    <span
                      className={`submenu-indicator ${isOpen ? "open" : ""}`}
                    >
        ...
        <header className="admin-topbar">
          <div className="topbar-title-group">
            <div className="pwa-player-brand" aria-label="KontenMu">
              ...
              <span className="pwa-player-brand-text">
                Konten<span>Mu</span>
              </span>
            </div>
            <button
                            ...
                            >
                              <div style={{ display: "flex", gap: "8px" }}>
                                ...
                                <button
                                  disabled={approvingIds.has(user.id)}
                                  style={{
                                    background: approvingIds.has(user.id)
                                      ? "#94a3b8"
                                      : "var(--accent-primary)",
                                    color: "white",
                                    border: "none",
                                    ...
                                    padding: "6px 12px",
                                    fontSize: "0.8rem",
                                    cursor: approvingIds.has(user.id)
                                      ? "not-allowed"
                                      : "pointer",
                                    fontWeight: 600,
                                    minWidth: "72px",
                                  ...
                                  onClick={async () => {
                                    ...
                                    setApprovingIds(
                                      (prev) => new Set([...prev, user.id]),
                                    );

                                    // Hapus dari list lokal segera (optimistic)
                                    setPendingApiUsers((prev) =>
              ...
              <div style={{ position: "relative" }}>
                ...
                >
                  <div
                    ...
                    style={{
                      padding: session.picture ? 0 : undefined,
                      overflow: "hidden",
                    }}
                  >
                    {session.picture ? (
      ...
      >
        ...
        <button
          className="mobile-bottom-nav-item mobile-more-button"
          type="button"
          onClick={() => setSidebarOpen(true)}
        >
          <Icon name="menu" />
      ...
      {(session?.role === "sekolah" || session?.role === "siswa") &&
        data.isChatWidgetEnabled && <SchmuChatWidget />}
    </div>
  );
}
