...
export default function SalesForm() {
  ...
  const unitPrice = Math.round(basePrice * packageMultipliers[paket] * durationMultipliers[durasi]);
  const subtotal = quantity * unitPrice;
  const discount = Math.round(subtotal * (Number(diskonPersen || 0) / 100));
  const afterDiscount = Math.max(subtotal - discount, 0);
  const commission = Math.round(afterDiscount * (Number(komisiPersen || 0) / 100));
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    setData((current) => {
      ...
      const nextSubscription = {
        ...
        requestAt: startedAt,
        agentDeadline: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10),
      ...
      const nextPayment = {
        ...
        schoolId: Number(schoolId),
        nominal: invoiceTotal,
        jatuhTempo: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10),
    ...
    setMessage(`Invoice ${invoiceNo} berhasil dibuat. Komisi agen: ${formatCurrency(commission)}. Langganan ${durasi} tersimpan menunggu approval.`);
    setJumlah('');
  };
      ...
      <GlassCard style={{ width: '100%', maxWidth: '980px' }}>
        <div className="panel-heading">
          <div>
            ...
            <p>Hitung harga paket, diskon, komisi agen, periode subscribe 3/6/12 bulan, dan invoice untuk kebutuhan penjualan.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="form-stack">
          <div className="form-grid">
            ...
            <label>
              Paket Penjualan
              <select className="input-control" value={paket} onChange={(e) => setPaket(e.target.value as SalesPackage)}>
            ...
            <label>
              Durasi Subscribe
              <select className="input-control" value={durasi} onChange={(e) => setDurasi(e.target.value as SubscriptionDuration)}>
                {(['3 Bulan', '6 Bulan', '1 Tahun'] as SubscriptionDuration[]).map((option) => <option key={option} value={option}>{option}</option>)}
              </select>
            </label>

            <label>
              Jumlah Lisensi / Eksemplar
              <input className="input-control" type="number" min={1} value={jumlah} onChange={(e) => setJumlah(e.target.value)} placeholder="Contoh: 150" required />
            </label>

            <label>
              Diskon (%)
              <input className="input-control" type="number" min={0} max={100} value={diskonPersen} onChange={(e) => setDiskonPersen(e.target.value)} />
            </label>

            {isSuperAdmin && (
              <label>
                Komisi Agen (%)
                <input className="input-control" type="number" min={0} max={100} value={komisiPersen} onChange={(e) => setKomisiPersen(e.target.value)} />
              </label>
            )}
          </div>
          ...
          <div className="invoice-summary-grid">
            ...
            <SummaryItem label="Harga Satuan Paket" value={formatCurrency(unitPrice)} />
            <SummaryItem label="Subtotal" value={formatCurrency(subtotal)} />
            <SummaryItem label={`Diskon ${Number(diskonPersen || 0)}%`} value={`-${formatCurrency(discount)}`} />
            <SummaryItem label="Total Invoice" value={formatCurrency(invoiceTotal)} highlight />
            {isSuperAdmin && <SummaryItem label={`Komisi Agen ${Number(komisiPersen || 0)}%`} value={formatCurrency(commission)} />}
          </div>
