...
const ImageUploadField = ({ value, onChange }: { value: any, onChange: (val: any) => void }) => {
  const [isUploading, setIsUploading] = useState(false);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      {value && <img src={value} alt="Preview" style={{ width: '100%', maxHeight: '150px', objectFit: 'contain', borderRadius: '4px', border: '1px solid #e5e7eb' }} />}
      {isUploading && <div style={{ fontSize: '12px', color: '#6b7280' }}>Mengunggah...</div>}
      ...
      <button 
        ...
        style={{ padding: '6px', background: '#fee2e2', color: '#ef4444', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', fontWeight: 600, opacity: isUploading ? 0.5 : 1 }}
      >
        Hapus / Pakai Default
      </button>
    </div>
...
const ColorPickerField = ({ value, onChange }: { value: any, onChange: (val: any) => void }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
    <input 
      type="color" 
      value={value || '#ffffff'} 
      onChange={(e) => onChange(e.target.value)} 
      style={{ width: '32px', height: '32px', padding: 0, border: '1px solid #e5e7eb', borderRadius: '4px', cursor: 'pointer' }}
    />
    <input 
      type="text" 
      value={value || ''} 
...
const TestimonialsComponent = ({ kicker, title, items, paddingTop, paddingBottom }: any) => {
  const safeItems = items || [
    {
      quote: "Distribusi konten pendidikan jadi jauh lebih rapi. Tim sekolah tidak lagi berpindah-pindah sistem. Stabilitas sistem saat menangani aktivasi siswa sangat lancar berkat arsitektur headless ini.",
      name: "Ratna Sari",
      role: "Admin Sekolah",
    },
    {
      quote: "Sinkronisasi materi ajar berjalan tanpa cela. Guru tetap nyaman menyusun presentasi, sementara siswa menikmati pengalaman belajar yang super cepat dan interaktif di kelas.",
      name: "Bima Prakoso",
      role: "Guru Penggerak",
    },
    {
      quote: "Akses aset video pembelajaran dan game edukasi kini sangat efisien. Masalah buffering yang sering terjadi saat jam belajar sibuk sudah teratasi sepenuhnya.",
      name: "Ahmad Fauzi",
  ...
  const slideLeft = () => {
    if (sliderRef.current) {
      const cardWidth = (sliderRef.current.querySelector('.antigravity-card') as HTMLElement)?.offsetWidth || 400;
  ...
  return (
    <section id="testimoni" className="md-testimonials" style={{ paddingTop: getPadding(paddingTop), paddingBottom: getPadding(paddingBottom) }}>
      <div className="md-container">
        <div className="md-testimonial-header">
          ...
          <div className="md-slider-nav">
            <button onClick={slideLeft} className="md-nav-btn antigravity-btn group" aria-label="Previous">
              <svg className="w-6 h-6 md-icon-left" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7"></path>
              </svg>
            </button>
            <button onClick={slideRight} className="md-nav-btn antigravity-btn group" aria-label="Next">
              <svg className="w-6 h-6 md-icon-right" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div id="testimonial-slider" className="md-slider-track no-scrollbar" ref={sliderRef}>
          {safeItems.map((item: any, idx: number) => (
            <div key={idx} className={`md-testimonial-card antigravity-card ${idx === 1 ? 'md-card-dark' : 'md-card-light'}`}>
              ...
              <div className="md-author">
                <div className="md-author-avatar" style={item.avatar ? { backgroundImage: `url(${item.avatar})`, backgroundSize: 'cover' } : {}}></div>
                <div>
...
const RootComponent = ({ children, paddingTop, paddingBottom, headerTitle, showFooter, footerText, navItems, headerLogo, headerColor, customCSS, headerBtnText, headerBtnLink, favicon, seoTitle, seoDescription, seoImage }: any) => {
  ...
  return (
    <div className="md-landing-wrapper" style={{ paddingTop: getPadding(paddingTop), paddingBottom: getPadding(paddingBottom) }}>
      {customCSS && <style>{customCSS}</style>}
      {/* HEADER */}
      <header className={`md-header ${!headerColor ? 'glass-nav' : ''}`} style={headerColor ? { backgroundColor: headerColor } : {}}>
        <div className="md-container md-header-inner">
          <Link to="/" className="md-brand" style={{textDecoration: 'none'}}>
            {headerLogo ? (
              <img src={headerLogo} alt="Logo" style={{ height: '32px', objectFit: 'contain' }} />
            ) : (
          ...
          <Link to={headerBtnLink || '/login'} className="md-btn-primary antigravity-btn">
        ...
        <footer className="md-footer">
          <div className="md-container md-footer-inner">
            <div className="md-brand-footer">
              {headerLogo ? (
                <img src={headerLogo} alt="Logo" style={{ height: '24px', objectFit: 'contain' }} />
              ) : (
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <p className="md-footer-copy">{footerText || '© 2026 KontenMu. All rights reserved.'}</p>
              <Link to="/editor" style={{ color: 'var(--text-tertiary)', textDecoration: 'none', fontSize: '12px', opacity: 0.5 }}>Editor</Link>
            </div>
          </div>
        </footer>
      )}
  ...
  components: {
    Hero: {
      ...
      defaultProps: {
        ...
        description: 'Kombinasi fleksibilitas pengelolaan perpustakaan sekolah dan kecepatan absolut bagi siswa. Dirancang khusus untuk mendigitalkan materi ajar secara interaktif dan terpusat.',
        btnText: 'Mulai Demo Gratis',
        btnLink: '/login',
      },
      render: ({ badgeText, title1, titleGradient, description, btnText, btnLink, heroImage, paddingTop, paddingBottom }) => (
        <section className="md-hero" style={{ paddingTop: getPadding(paddingTop), paddingBottom: getPadding(paddingBottom) }}>
          ...
          <div className="md-container md-hero-content">
            ...
            <div className="md-hero-visual-wrapper">
              <div className="md-hero-visual antigravity-card">
                <img src={heroImage || dashboardPreview} alt="Dashboard Preview" className="md-hero-img" />
              </div>
            </div>
          </div>
        </section>
    ...
    SocialProof: {
      ...
      defaultProps: {
        ...
        clients: [{ name: 'Dikdasmen' }, { name: 'PWM Jateng' }, { name: 'LabMu' }, { name: 'KontenMu Press' }],
      },
      render: ({ title, clients, paddingTop, paddingBottom }) => {
        const safeClients = clients || [{ name: 'Dikdasmen' }, { name: 'PWM Jateng' }, { name: 'LabMu' }, { name: 'KontenMu Press' }];
        return (
        <section id="klien" className="md-social-proof" style={{ paddingTop: getPadding(paddingTop), paddingBottom: getPadding(paddingBottom) }}>
          <div className="md-container">
            ...
            <div className="md-client-row">
              {safeClients.map((client, i) => {
                ...
                return (
                <span key={i} className="md-client-logo" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                  {finalLogo && (
                    <img src={finalLogo} alt={client.name} style={{ maxHeight: '40px', objectFit: 'contain' }} />
                  )}
    ...
    Feature: {
        ...
        features: [
          {
            ...
            description: 'Kami mengintegrasikan sistem dengan infrastruktur visual interaktif. Pengelolaan aset Game HTML5 dan video resolusi tinggi kini tidak lagi membebani jaringan sekolah Anda.',
            iconText: 'Materi Interaktif & Multimedia',
            isReverse: false,
          },
          {
            subtitle: 'Manajemen Terpusat & Lisensi',
            description: 'Memanfaatkan dashboard mutakhir untuk memantau alur distribusi agen. Mengatur aktivasi langganan sekolah dan siswa kini menjadi transparan secara *real-time*.',
            iconText: '',
            isReverse: true,
          }
        ]
      ...
      render: ({ kicker, title, features, paddingTop, paddingBottom }) => {
        ...
        return (
        <section id="fitur" className="md-features" style={{ paddingTop: getPadding(paddingTop), paddingBottom: getPadding(paddingBottom) }}>
          <div className="md-container">
            ...
            {safeFeatures.map((feat, i) => (
              <div key={i} className={`md-feature-row ${feat.isReverse ? 'md-feature-reverse' : ''}`}>
                {!feat.isReverse ? (
                  <>
                    ...
                    <div className="md-feature-box antigravity-card">
                      {feat.image ? (
                        <img src={feat.image} alt={feat.subtitle} style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: 'inherit' }} />
                      ) : (
                        <div className="md-feature-box-inner">
                          <svg className="md-feature-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    CTA: {
      ...
      defaultProps: {
        ...
        description: 'Jangan biarkan keterbatasan distribusi menghambat potensi sekolah Anda. Bangun ekosistem belajar performa tinggi dan kelola dengan transparan hari ini.',
        btnText: 'Mulai Transformasi Sekarang',
        btnLink: '/login',
        footerText: 'Memberdayakan Sekolah di Seluruh Nusantara.',
      },
      render: ({ title, description, btnText, btnLink, footerText, paddingTop, paddingBottom }) => (
        <section id="cta" className="md-cta" style={{ paddingTop: getPadding(paddingTop), paddingBottom: getPadding(paddingBottom) }}>
    ...
    CustomHTML: {
      ...
      render: ({ html, paddingTop, paddingBottom }) => (
        <section style={{ paddingTop: getPadding(paddingTop), paddingBottom: getPadding(paddingBottom) }} dangerouslySetInnerHTML={{ __html: html || '' }} />
      )
    },
  },
  root: {
