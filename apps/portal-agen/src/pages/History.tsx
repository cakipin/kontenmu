              ...
              {paginatedSales.map((row) => {
                ...
                return (
                  <tr key={row.id}>
                    ...
                    <td>
                      {row.paket
                        ? `${row.paket}${row.durasiBulan ? ` · ${row.durasiBulan} bln` : ""}`
                        : (book?.judul ?? row.isbn)}
                    </td>
                    <td style={{ textAlign: "center" }}>
                      {formatNumber(row.jumlah)} Lisensi
                    </td>
                    <td style={{ textAlign: "right" }}>
                      {formatCurrency(saleSubtotal(row, book))}
                    </td>
                    <td style={{ color: "var(--warning)", textAlign: "right" }}>
                      {formatCurrency(saleDiscount(row, book))}
                    </td>
                    <td
                      style={{
                    ...
                    >
                      {formatCurrency(saleCommission(row, book))}
                    </td>
                    <td style={{ textAlign: "right" }}>
                      {formatCurrency(saleInvoiceTotal(row, book))}
                    </td>
                    <td style={{ textAlign: "center" }}>
                      <Chip
                        type="success"
                        label={new Date(row.tanggal).toLocaleDateString(
                          "id-ID",
                        )}
                      />
                    </td>
                    <td style={{ textAlign: "center" }}>
                      ...
                      >
                        ...
                        <button
                          className="icon-button"
                          style={{ color: "var(--warning)", padding: "6px" }}
            ...
            <div id="print-area" style={{ color: "var(--text-primary)" }}>
              <div style={{ textAlign: "center", marginBottom: "32px" }}>
                ...
                <p style={{ margin: 0, color: "#64748b" }}>
                  Nomor:{" "}
                  {previewRow.invoiceNo ??
                    `#${previewRow.id.toString().padStart(4, "0")}`}
                </p>
                <p style={{ margin: 0, color: "#64748b" }}>
                  Tanggal:{" "}
                  {new Date(previewRow.tanggal).toLocaleDateString("id-ID")}
                </p>
              </div>
              ...
              <div style={{ marginBottom: "32px" }}>
                ...
                >
                  {getSchool(data, previewRow.schoolId)?.nama ?? "-"}
                </p>
                <p style={{ margin: 0, color: "#64748b" }}>
                  {getSchool(data, previewRow.schoolId)?.kota ?? "-"}
                </p>
              </div>
                ...
                <tbody>
                  <tr style={{ borderBottom: "1px solid #e2e8f0" }}>
                    <td style={{ padding: "16px 0" }}>
                      {previewRow.paket
                        ? `${previewRow.paket} (${previewRow.durasiBulan} bulan)`
                        : (getBook(data, previewRow.isbn)?.judul ??
                          previewRow.isbn)}
                    </td>
                    <td style={{ textAlign: "center", padding: "16px 0" }}>
                      {formatNumber(previewRow.jumlah)} Lisensi
                    </td>
                    <td style={{ textAlign: "right", padding: "16px 0" }}>
                      {formatCurrency(
                        saleSubtotal(
                          previewRow,
                          getBook(data, previewRow.isbn),
                        ),
                      )}
                    </td>
                  </tr>
      ...
      {editRow && (
        ...
        >
          ...
          >
            ...
            >
              Fitur Edit transaksi penjualan akan terintegrasi langsung dengan
              formulir <strong>Input Penjualan</strong> pada pembaruan sistem
              berikutnya. Saat ini data bersifat <em>read-only</em> untuk
        ...
        >
          ...
          >
            ...
            >
              ...
              <strong>{deleteRow.invoiceNo ?? `#${deleteRow.id}`}</strong>?
            ...
            >
              ...
              <button
                onClick={() => {
                  setData((prev) => ({
                    ...prev,
                    sales: prev.sales.filter((s) => s.id !== deleteRow.id),
                  }));
                  setDeleteRow(null);
                }}
                style={{
